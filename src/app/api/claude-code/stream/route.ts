...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const base of BACKEND_BASE_CANDIDATES) {
      try {
        response = await fetchWithRetry(`${base}/claude-code/stream`, {
          method: 'POST',
    ...
    // Create a streaming response with proper timeout handling
    ...
    console.error('Error calling Claude Code stream API:', error)
    return NextResponse.json(
      { error: 'Failed to call Claude Code stream API' },
      { status: 500 }
    )
  }
}