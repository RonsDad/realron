...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const base of BACKEND_BASE_CANDIDATES) {
      try {
        response = await fetchWithRetry(`${base}/claude-code/execute`, {
          method: 'POST',
    ...
    console.error('Error calling Claude Code execute API:', error)
    return NextResponse.json(
      { error: 'Failed to call Claude Code execute API' },
      { status: 500 }
    )
  }
}