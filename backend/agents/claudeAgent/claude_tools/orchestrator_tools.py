...
class AgentOrchestrator:
    ...
    def __init__(self):
        ...
        self.agent_sessions: Dict[str, Dict[str, Any]] = {}  # Track agent sessions
        self.session_cleanup_tasks: Dict[str, asyncio.Task] = {}  # Auto-cleanup tasks
    ...
    async def spawn_agent(
        ...
        try:
            ...
            cleanup_task = asyncio.create_task(
                self._schedule_cleanup(agent_id, session_id, timeout=1800)
            )
            self.session_cleanup_tasks[session_id] = cleanup_task

            logger.info(
                f"✅ Agent {agent_id} spawned with {len(allowed_tools)} tools and {thinking_budget} thinking budget"
            )
    ...
    async def execute_agent(self, agent_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            async for event in self.claude.stream_complete(
                ...
                thinking_budget=agent_config.get(
            ...
            session_id = self.active_agents[agent_id].get("session_id")
            if session_id and session_id in self.agent_sessions:
                self.agent_sessions[session_id]["status"] = "completed"
                self.agent_sessions[session_id]["end_time"] = datetime.now()
                self.agent_sessions[session_id]["tool_calls"] = used_tools
                self.agent_sessions[session_id]["events"] = all_events

                # Schedule cleanup after 5 minutes for completed agents
                if session_id in self.session_cleanup_tasks:
                    self.session_cleanup_tasks[session_id].cancel()
                cleanup_task = asyncio.create_task(
                    self._schedule_cleanup(agent_id, session_id, timeout=300)
                )
                self.session_cleanup_tasks[session_id] = cleanup_task

            logger.info(
                f"✅ Agent {agent_id} completed with enhanced thinking ({len(thinking_content)} thinking tokens)"
            )
    ...
    def _get_agent_prompt(self, agent_type: str) -> str:
        ...
        return prompts.get(
            agent_type,
            f"You are a {agent_type}. Complete your assigned task professionally.",
        )
    ...
    async def execute_parallel_agents(self, agent_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"✅ Parallel execution: {successful} successful, {failed} failed, {total_thinking_tokens} thinking tokens"
            )
    ...
    async def cleanup_agent(self, agent_id: str) -> Dict[str, Any]:
        """Remove agent from memory and clean up all associated resources."""
        if agent_id in self.active_agents:
            # Get session ID
            session_id = self.active_agents[agent_id].get("session_id")

            # Cancel any pending cleanup tasks
            if session_id and session_id in self.session_cleanup_tasks:
                self.session_cleanup_tasks[session_id].cancel()
                del self.session_cleanup_tasks[session_id]

            # Clean up session tracking
            if session_id and session_id in self.agent_sessions:
                self.agent_sessions[session_id]["status"] = "cleaned"
                self.agent_sessions[session_id]["cleanup_time"] = datetime.now()
                # Archive session for debugging (keep for 1 hour)
                asyncio.create_task(self._archive_session(session_id))

            # Remove from active agents
            del self.active_agents[agent_id]

            # Clear from shared memory
            if (
                hasattr(self.shared_memory, "agent_outputs")
                and agent_id in self.shared_memory.agent_outputs
            ):
                del self.shared_memory.agent_outputs[agent_id]

            logger.info(
                f"✅ Agent {agent_id} and session {session_id} fully cleaned up"
            )
            return {
                "success": True,
                "message": f"Agent {agent_id} cleaned up",
                "session_id": session_id,
            }
        return {"success": False, "error": f"Agent {agent_id} not found"}

    async def _schedule_cleanup(
        self, agent_id: str, session_id: str, timeout: int = 1800
    ):
        """Schedule automatic cleanup of agent after timeout."""
        try:
            await asyncio.sleep(timeout)
            logger.info(f"⏰ Auto-cleaning agent {agent_id} after {timeout}s timeout")
            await self.cleanup_agent(agent_id)
        except asyncio.CancelledError:
            logger.debug(f"Cleanup task for {agent_id} was cancelled")

    async def _archive_session(self, session_id: str):
        """Archive session data for debugging, then remove after 1 hour."""
        await asyncio.sleep(3600)  # Keep for 1 hour
        if session_id in self.agent_sessions:
            del self.agent_sessions[session_id]
            logger.debug(f"Archived session {session_id} removed")

    async def get_all_sessions(self) -> Dict[str, Any]:
        """Get status of all agent sessions for monitoring."""
        return {
            "success": True,
            ...
            "sessions": [
                {
                    ...
                    "tool_calls_count": len(session.get("tool_calls", [])),
                    "events_count": len(session.get("events", [])),
                }
                for sid, session in self.agent_sessions.items()
            ],
        }
    ...
    async def end_all_sessions(self) -> Dict[str, Any]:
        ...
        for agent_id in agent_ids:
            result = await self.cleanup_agent(agent_id)
            cleanup_results.append({"agent_id": agent_id, "result": result})

        return {
            "success": True,
            "message": f"Cleaned up {len(cleanup_results)} agents",
...
async def execute_agent_pipeline(
    chain_name: str, initial_task: str, context: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
    """
...
async def spawn_healthcare_agent(
    ...
    system_prompt = system_prompts.get(
        specialty, f"You are a {specialty} healthcare specialist agent."
    )
...
async def orchestrate_healthcare_task(
    ...
    try:
        ...
        for agent_id, result in execution_result.get("results", {}).items():
            if result.get("success") and result.get("json"):
                ...
                if "estimated_savings" in data and data["estimated_savings"]:
                    try:
                        aggregated["estimated_savings_total"] += float(
        ...
        logger.info(
            f"✅ Healthcare orchestration completed: {len(agent_outputs)} agents, {total_thinking_tokens} thinking tokens"
        )
