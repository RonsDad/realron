...
from backend.agents.claudeAgent.claude_tools.browser_use.browser_live_url_manager import (
    live_url_manager,
)
...
try:
    from backend.agents.claudeAgent.claude_tools.clinical_agent.healthcare_agent_integration import (
        HealthcareAgentIntegration,
    )
    from backend.agents.claudeAgent.claude_tools.browser_use.claude_browser_integration import (
        ClaudeBrowserIntegration,
    )
...
try:
    from backend.agents.claudeAgent.claude_tools.claude_code_sdk.api_integration import (
        claude_code_api,
    )
...
if True:  # Always enable
    try:
        ...
        from backend.agents.deepResearch.deep_research_integration import (
            root_agent as deep_research_root_agent,
        )
        ...
        if DEEP_RESEARCH_AVAILABLE:
            ...
            logger.warning(
                "Deep Research Agent wrapper imported but no root_agent was found"
            )
            deep_research_root_agent = None
    except Exception as e:
...
app.add_middleware(
    ...
    expose_headers=["Content-Type", "Cache-Control", "Connection", "Keep-Alive"],
)
...
class ChatRequest(BaseModel):
    ...
    stream: bool = True  # Default to streaming for real-time responses
...
async def create_browser_agent(task: str, session_id: str):
    ...
    try:
        ...
        await ws_manager.broadcast(
            session_id, json.dumps({"event": "session_started", "live_url": live_url})
        )
...
async def stop_recording_and_close(session_id: str):
    ...
    if service_session_id:
        try:
            ...
            await ws_manager.broadcast(
                session_id, json.dumps({"event": "session_stopped"})
            )
        except Exception as e:
...
async def run_agent_safe(session, session_id):
    ...
    try:
        ...
        await ws_manager.broadcast(
            session_id, json.dumps({"event": "agent_run_complete"})
        )
    except Exception as e:
        logging.error(f"Error running agent for session {session_id}: {e}")
        await ws_manager.broadcast(
            session_id, json.dumps({"event": "agent_error", "error": str(e)})
        )
    finally:
...
async def run_browser_use_agent(task: str):
    ...
    try:
        ...
        if active_sessions["total_sessions"] > 0:
            ...
            session_result = await browser_use_service.create_live_url_session(
                timeout_ms=900000, browser_profile=profile, interactive=False
            )
    ...
    return {
        ...
        "endpoints": {
            ...
            "deep_research_session": (
                "/api/apps/{app}/users/{user}/sessions"
                if DEEP_RESEARCH_AVAILABLE
                else None
            ),
            "browser_use_create_session": (
                "/browser-use/session/create" if BROWSER_USE_AVAILABLE else None
            ),
            "browser_use_create_with_url": (
                "/browser-use/session/create-with-url"
                if BROWSER_USE_AVAILABLE
                else None
            ),
            "browser_use_navigate": (
                "/browser-use/session/{session_id}/navigate"
                if BROWSER_USE_AVAILABLE
                else None
            ),
            "browser_use_task": (
                "/browser-use/session/{session_id}/task"
                if BROWSER_USE_AVAILABLE
                else None
            ),
            "browser_use_info": (
                "/browser-use/session/{session_id}/info"
                if BROWSER_USE_AVAILABLE
                else None
            ),
            "browser_use_sessions": (
                "/browser-use/sessions" if BROWSER_USE_AVAILABLE else None
            ),
            "browser_use_close": (
                "/browser-use/session/{session_id}/close"
                if BROWSER_USE_AVAILABLE
                else None
            ),
            "browser_use_close_all": (
                "/browser-use/sessions/close-all" if BROWSER_USE_AVAILABLE else None
            ),
            "tool_generate": "/tools/generate" if CLAUDE_CODE_SDK_AVAILABLE else None,
            "tool_generate_stream": (
                "/tools/generate/stream" if CLAUDE_CODE_SDK_AVAILABLE else None
            ),
            "tool_get": "/tools/{tool_id}" if CLAUDE_CODE_SDK_AVAILABLE else None,
            "tool_share": (
                "/tools/share/{tool_id}" if CLAUDE_CODE_SDK_AVAILABLE else None
            ),
            "tool_list": (
                "/tools/patient/{patient_id}" if CLAUDE_CODE_SDK_AVAILABLE else None
            ),
...
async def chat(request: ChatRequest):
    ...
    logger.info(
        f"Request config: stream={request.stream}, thinking={request.enable_thinking}, tools={request.tools}"
    )
    ...
    try:
        if claude_agent is None:
            raise HTTPException(
                status_code=503,
                detail="Claude agent not initialized. Check ANTHROPIC_API_KEY.",
            )
        ...
        logger.debug(
            f"Final tools - native: {native_tools}, custom: {[t.get('name') for t in enabled_tools]}"
        )
        ...
        if request.stream:
            ...
            async def stream_generator():
                try:
                    ...
                    if live_url_manager and hasattr(
                        live_url_manager, "set_event_queue"
                    ):
        ...
        if len(enabled_tools) > 10:
            ...
            from backend.agents.claudeAgent.claude_tools.claude_tool_handler import (
                handle_tool_use_in_conversation,
            )
...
async def initialize_computer_use():
    ...
    """
    try:
        from backend.agents.claudeAgent.claude_tools.computer_use.vnc_computer_handler import (
            computer_handler,
        )
...
async def test_mcp_connector(request: MCPConnectorTestRequest):
    ...
    if not anth_key:
        return MCPConnectorTestResponse(
            success=False, reason="Missing ANTHROPIC_API_KEY in environment"
        )

    server_url = request.server_url or os.getenv("TELNYX_MCP_URL")
    if not server_url:
        return MCPConnectorTestResponse(
            success=False, reason="Missing TELNYX_MCP_URL (server_url)"
        )

    auth_token = (
        request.authorization_token
        or os.getenv("TELNYX_MCP_TOKEN")
        or os.getenv("TELNYX_API_KEY")
    )
    if not auth_token:
        return MCPConnectorTestResponse(
            success=False, reason="Missing TELNYX_MCP_TOKEN or TELNYX_API_KEY"
        )
    ...
    try:
        ...
        return MCPConnectorTestResponse(
            success=True, http_status=200, raw=resp.model_dump()
        )
    except anthropic.APIError as e:
        return MCPConnectorTestResponse(
            success=False, reason=str(e), http_status=getattr(e, "status_code", None)
        )
    except Exception as e:
...
async def deep_research_sse(request: DeepResearchRequest):
    ...
    logger.info(
        f"Request: sessionId={request.sessionId}, userId={request.userId}, message={request.message[:100]}..."
    )
...
5. Consider multiple perspectives on the topic"""
            ...
            async def event_stream():
                async for event in claude_agent.stream_complete(
                    ...
                    sse_event = {
                        "content": {
                            "parts": [
                                {"text": getattr(event, "delta", {}).get("text", "")}
                            ],
                            "role": "model",
                        },
    ...
    try:
        logger.info(
            f"Deep research request received - sessionId: {request.sessionId}, userId: {request.userId}"
        )
        ...
        async def event_stream():
            try:
                ...
                async for event in agent.run_sse(message_text):
                    ...
                    if event_count % 10 == 0:
                        logger.info(
                            f"Processed {event_count} events from deep research agent"
                        )
                    ...
                    event_data = {
                        ...
                        "longRunningToolIds": [],
                        "id": (
                            event.get("id", f"event_{datetime.now().timestamp()}")
                            if isinstance(event, dict)
                            else f"event_{datetime.now().timestamp()}"
                        ),
                        "timestamp": datetime.now().timestamp(),
                    }
                    ...
                    if event_type == "complete" and isinstance(event, dict):
                        ...
                        if result.get("success"):
                            ...
                            event_data["hallucination_check"] = result.get(
                                "hallucination_check", {}
                            )
                        ...
                        if agents_active:
                            event_data["content"] = (
                                f"{message} - Active: {', '.join(agents_active)}"
                            )
                        else:
                ...
                logger.error(
                    f"Error in deep research event stream: {str(e)}", exc_info=True
                )
                error_event = {
                    "content": {
        ...
        raise HTTPException(
            status_code=500, detail=f"Deep Research Agent error: {str(e)}"
        )
...
from backend.agents.claudeAgent.claude_tools.telnyx_webhook_handler import (
    router as telnyx_webhook_router,
)
...
try:
    from backend.agents.claudeAgent.claude_tools.telnyx_mcp_gateway import (
        router as telnyx_mcp_gateway_router,
    )
...
# Browserbase Session Handler for live session URL integration
try:
    from backend.api.browserbase_session_handler import router as browserbase_router

    app.include_router(browserbase_router, tags=["Browserbase Sessions"])
    logger.info("Browserbase session handler mounted at /api/browserbase/...")
except Exception as e:
    logger.warning(f"Failed to mount Browserbase session handler: {e}")

# Agent Output Endpoints for captured agent visibility
try:
    from backend.api.agent_output_endpoints import router as agent_output_router

    app.include_router(agent_output_router, tags=["Agent Outputs"])
    logger.info("Agent output endpoints mounted for captured agent visibility")
except Exception as e:
    logger.warning(f"Failed to mount agent output endpoints: {e}")
...
async def test_browserless_connection(request: Request):
    ...
    if not BROWSER_USE_AVAILABLE:
        raise HTTPException(
            status_code=503, detail="Browser-use integration is not available"
        )
    ...
    if not browserless_token:
        raise HTTPException(
            status_code=422,
            detail="BROWSERLESS_API_TOKEN environment variable is required",
        )

    anthropic_api_key = os.getenv("ANTHROPIC_API_KEY")
    if not anthropic_api_key:
        raise HTTPException(
            status_code=422, detail="ANTHROPIC_API_KEY environment variable is required"
        )
...
async def create_browser_use_session_with_url(request: BrowserUseCreateWithURLRequest):
    ...
    try:
        from backend.agents.claudeAgent.claude_tools.browser_use.browser_use_service import (
            create_browser_session_with_url,
        )

        result = await create_browser_session_with_url(request.url, request.timeout_ms)

        logger.info(
            f"Created browser-use session {result['session_id']} and navigated to {request.url}"
        )
...
async def navigate_browser_use_session(
    session_id: str, request: BrowserUseNavigateRequest
):
    """
    ...
    try:
        result = await browser_use_service.navigate_and_get_live_url(
...
async def execute_browser_use_task(session_id: str, request: BrowserUseTaskRequest):
    ...
    try:
        result = await browser_use_service.execute_browser_task(
            session_id, request.task
        )
...
async def take_control(session_id: str):
    ...
    if session:
        ...
        await ws_manager.broadcast(
            session_id, json.dumps({"event": "human_in_control"})
        )
        return {"status": "ok"}
...
async def relinquish_control(session_id: str):
    ...
    if session:
        ...
        await ws_manager.broadcast(
            session_id, json.dumps({"event": "agent_in_control"})
        )
...
async def websocket_endpoint(websocket: WebSocket, session_id: str):
    ...
    try:
        while True:
            ...
            await ws_manager.broadcast(
                session_id, json.dumps({"event": "echo", "data": data})
            )
...
async def startup_event():
    ...
    logger.info(
        "Telnyx MCP will be connected via Claude's native MCP connector when needed"
    )
